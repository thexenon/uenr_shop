...
class UserDatabaseHelper {
  ...
  Future<bool> addProductToCart(String productId) async {
    ...
    final cartCollectionRef = firestore
        ...
        .doc(uid)
        .collection(CART_COLLECTION_NAME);
    final productsDocsList =
        await firestore.collection("products").doc(productId).get();
    final ownerUID = productsDocsList.data()['owner'];
    final docRef = cartCollectionRef.doc(productId);
    final docSnapshot = await docRef.get();
    bool alreadyPresent = docSnapshot.exists;
    if (alreadyPresent == false) {
      docRef.set(CartItem(itemCount: 1, owner: ownerUID).toMap());
    } else {
      docRef.update(
        {
          CartItem.ITEM_COUNT_KEY: FieldValue.increment(1),
        },
      );
    }
    return true;
  ...
  Future<List<Map>> emptyCart() async {
    ...
    final userInfo = userRef.data();
    List<Map> orderedProductsMap = [];
    for (final doc in cartItems.docs) {
      final docData = doc.data();
      orderedProductsMap.add({
        "id": doc.id,
        "owner": docData['owner'],
        "number": docData['item_count'].toString(),
